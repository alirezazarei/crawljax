...
public class StateFlowGraph implements Serializable {
	...
	public static Eventable deserializeEventable (byte [] serializedEventable, int i)
	{
		Eventable results = (Eventable) SerializationUtils.deserialize(serializedEventable);
...
//					.getStrippedDom()));
	...
	public Set<Eventable> getOutgoingClickables(StateVertex stateVertix) {
		...
		for (Relationship edge: state.getRelationships(RelTypes.TRANSITIONS_TO, Direction.OUTGOING)){
			...
			Eventable eventable = (Eventable) deserializeEventable( serializedEvantable,1);
